...
- Quartz 기반 스케줄링 적용 시 다음 원칙을 따른다.
  - Quartz Job에는 `taskId`만 전달한다.
  - 실행 시 `taskId`로 DB에서 스케줄/설정 정보를 로딩한다.
  - Cron 변경은 Quartz Trigger **reschedule**로 즉시 반영한다.
  - 실행 직전 DB **lease 락**으로 중복 실행을 차단한다.
- **영속성 확보**:
  - Quartz JobStore는 **JDBC JobStore**를 사용하여 인스턴스 재시작 간 스케줄 소실을 방지한다.
  - 또는 애플리케이션 시작 시 DB 스케줄 정보를 읽어 Quartz와 동기화하는 로직을 포함한다.
