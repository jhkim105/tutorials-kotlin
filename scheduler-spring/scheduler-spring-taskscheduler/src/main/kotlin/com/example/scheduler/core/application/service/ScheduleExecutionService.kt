...
class ScheduleExecutionService(
    ...
    override fun runDueSchedules() {
        ...
        val schedules =
                scheduleRepository.lockDueSchedules(
                        now,
    ...
    override fun runSchedule(scheduleId: String) {
        val now = clock.instant()
        val locked =
                scheduleRepository.tryLockSchedule(
                        scheduleId,
                        now,
                        now.plus(Duration.ofSeconds(scheduleLockSeconds))
                )
        if (!locked) {
            log.trace { "Could not acquire lock for schedule $scheduleId" }
            return
        }

        val schedule = scheduleRepository.findById(scheduleId)
        if (schedule == null) {
            scheduleRepository.releaseLock(scheduleId)
            return
        }
        ...
        try {
            ...
            log.error(ex) { "Failed to execute schedule $scheduleId" }
            scheduleRepository.releaseLock(scheduleId)
        }
    }
    ...
    private fun executeSchedule(schedule: Schedule) {
        ...
        val handler =
                taskRegistry.get(schedule.taskId)
                        ?: run {
        ...
        val execution =
                try {
                    ...
                    log.error(ex) {
                        "Failed to create running execution for schedule ${schedule.id}"
                    }
                    scheduleRepository.releaseLock(schedule.id)
                    return
                }
