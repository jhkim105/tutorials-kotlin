...
class H2DeviceRepository(
    ...
    override fun findAll(): List<Device> {
        logger.info { "Fetching all devices from local H2 database" }
    ...
    override fun findById(deviceId: String): Device? {
        logger.info { "Fetching device by ID $deviceId from local H2 database" }
    ...
    override fun save(device: Device): Device {
        logger.info { "Saving device ${device.deviceId} to local H2 database" }
        val sql = """
            INSERT INTO iot_devices (device_id, device_name, device_type, status, location, last_heartbeat) 
            VALUES (:deviceId, :deviceName, :deviceType, :status, :location, :lastHeartbeat)
        """.trimIndent()
        val params = mapOf(
            ...
            "lastHeartbeat" to Timestamp.from(device.lastHeartbeat)
        )
    ...
    override fun update(deviceId: String, device: Device): Device? {
        logger.info { "Updating device $deviceId in local H2 database" }
        val sql = """
            UPDATE iot_devices 
            SET device_name = :deviceName, device_type = :deviceType, status = :status, location = :location, last_heartbeat = :lastHeartbeat 
    ...
    override fun delete(deviceId: String): Boolean {
        logger.info { "Deleting device $deviceId from local H2 database" }
